    ...
    with arcpy.da.SearchCursor(ocena_JCWP_aPGW,['KOD_JCWP','S_P_eko'])as cur2:
        for row2 in cur2:
            if row2[1] in [u'BARDZO DOBRY', u'CO NAJMNIEJ DOBRY', u'DOBRY', u'DOBRY i POWYŻEJ DOBREGO', u'DOBRY I POWYŻEJ DOBREGO']:
                with arcpy.da.UpdateCursor(JCWP,kol)as cur:
                    for row in cur:
                        if row2[0] in [row[kol.index('ST_KOD_1')]] and row[kol.index('ST_KOD_2')]==u'' and row[kol.index('KROK')] is None:
    ...
    with arcpy.da.UpdateCursor(JCWP,kol)as cur:
        for row in cur:
            if row[kol.index('KROK')] is None:
                ...
                for st_kod in aPGW:
                    if st_kod !='':
                        with arcpy.da.SearchCursor(monit,['KOD_JCWP','KLASA_BIOL'])as cur2:
                            for row2 in cur2:
...
"""KROK 5_2017"""
def Krok5_2017(JCWP,ocena_JCWP_2017):
    print "realizuje Krok 5"
    start_time = time.time()
    kol=[]
    for field in arcpy.ListFields(JCWP):
    ...
    with arcpy.da.SearchCursor(ocena_JCWP_2017,['MS_KOD','STAN_POTENCJAL_EKO'])as cur2:
        for row2 in cur2:
            if row2[1] in [u'BARDZO DOBRY', u'CO NAJMNIEJ DOBRY', u'DOBRY', u'DOBRY i POWYŻEJ DOBREGO', u'DOBRY I POWYŻEJ DOBREGO']:
                with arcpy.da.UpdateCursor(JCWP,kol)as cur:
                    for row in cur:
                        if row2[0] in [row[kol.index('ST_KOD_1')]] and row[kol.index('ST_KOD_2')]==u'' and row[kol.index('KROK')] is None:
                            row[kol.index('KROK')]='5_2017'
                            cur.updateRow(row)
                del cur
    del cur2
    print "zakonczono Krok 5_2017 - ", round((time.time() - start_time)/60, 2), "min " # 0.53 min
    return

"""KROK 6_2017"""
def Krok6_2017(JCWP,ocena_JCWP_2017):
    print "realizuje Krok 6"
    start_time = time.time()
    kol=[]
    for field in arcpy.ListFields(JCWP):
            ...
            if row[kol.index('KROK')] is None:
                ...
                for st_kod in aPGW:
                    if st_kod !='':
                        ...
                        del cur2
                if 0 not in war and len(war)>0:
                    row[kol.index('KROK')]='6_2017'
                    cur.updateRow(row)
    del cur
    print "zakonczono Krok 6_2017 - ", round((time.time() - start_time)/60, 2), "min " # 4.85 min
    return

"""KROK 7_2017"""
def Krok7_2017(JCWP,ocena_JCWP_2017):
    print "realizuje Krok 7"
    start_time = time.time()
    kol=[]
    for field in arcpy.ListFields(JCWP):
    ...
    with arcpy.da.UpdateCursor(JCWP,kol)as cur:
        for row in cur:
            if row[kol.index('KROK')] is None:
                ...
                for st_kod in aPGW:
                    if st_kod !='':
                        with arcpy.da.SearchCursor(ocena_JCWP_2017,['MS_KOD','STAN_POTENCJAL_EKO','DL'])as cur2:
                            for row2 in cur2:
                                if row2[1] in [u'BARDZO DOBRY', u'CO NAJMNIEJ DOBRY', u'DOBRY', u'DOBRY i POWYŻEJ DOBREGO', u'DOBRY I POWYŻEJ DOBREGO'] and row2[0]==st_kod:
                                    war_dobry = war_dobry + row2[2]
                                elif row2[1] not in [u'BARDZO DOBRY', u'CO NAJMNIEJ DOBRY', u'DOBRY', u'DOBRY i POWYŻEJ DOBREGO', u'DOBRY I POWYŻEJ DOBREGO'] and row2[0]==st_kod:
                ...
                if war_dobry+war_zly==0:
                    ...
                    row[kol.index('KROK')]='7_2017'
                    cur.updateRow(row)
    del cur
    print "zakonczono Krok 7_2017 - ", round((time.time() - start_time)/60, 2), "min " # 4.94 min
    return
