...
def lokPktPrzek2Line(point_layer, id_przek, szerokosc_koryta):
    '''
    ...
    shp_path = os.path.join(output_dir, output_layer)

    arcpy.AddMessage("INFO. Tworzenie warstwy wynikowej {} w lokalizacji {}".format(output_layer, output_dir))

    arcpy.management.CreateFeatureclass(output_dir, output_layer, geometry_type='POLYLINE',
                                        spatial_reference='ETRS_1989_Poland_CS92')
    arcpy.management.AddField(shp_path, "ID_PRZEK", 'TEXT', field_length=30)

    arcpy.AddMessage(
        "INFO. Zapis warstwy liniowej przekroi korytowych na warstwie {}".format(output_layer))
...
if __name__ =='__main__':
    ...
    pkt = r'C:\robo\Przeglad_LOKAL_ROBO\KOMPLEKSOWA_OCENA\wyznaczane_przekroje.gdb\czarek_przekroje_pkt_P1_P2'
    rzeki = r'C:\robo\Przeglad_LOKAL_ROBO\SKOROWIDZE_WSZYSTKIE\SKOROWIDZE.gdb\MZPiMRP_skorowidz_rzeki_modele_2022v1'
    id_kol = 'ID_PRZEK'
    szer_koryta_kol = 'DL'

    calcRiverXnsCoord(pkt, rzeki, id_kol)
    lokPktPrzek2Line(pkt,id_kol,szer_koryta_kol)
